...
const UserRegistrations = () => {
  ...
  return (
    <Box sx={{ p: 4, mt: 8 }}>
      <Typography variant="h4" color="primary" sx={{ mb: 3 }}>My Registrations</Typography>
            ...
            {items.map((r) => (
              <Grid key={r._id} item xs={12} md={6}>
                <Paper sx={{ p: 2 }} variant="outlined">
                  ...
                  <Typography variant="body2" sx={{ mt: 1 }}>Dept: {r.department}{r.subOption ? ` (${r.subOption})` : ''}</Typography>
                  ...
                  <Typography variant="caption" display="block" sx={{ mt: 1, color: 'text.secondary' }}>Submitted: {new Date(r.createdAt).toLocaleString()}</Typography>
                </Paper>
              </Grid>
            ))}
          </Grid>
        ) : (
          <Typography color="text.secondary">You have not submitted any registrations yet.</Typography>
        )}
      </Paper>
